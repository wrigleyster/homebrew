...
def check_for_multiple_volumes
  ...
  tmp = Pathname.new `/usr/bin/mktemp -d #{tmp_prefix}/homebrew-brew-doctor-XXXX`.strip #TODO Mac -- /usr/bin/mktemp
...
def check_git_status
  ...
  if system "/usr/bin/which -s git" and File.directory? repo+'.git' and not `#{status_cmd}`.empty?   #TODO Mac
